...
describe("embed example page", () => {
  beforeEach(() => {
    vi.resetModules();
    mockSearchParams = new URLSearchParams();
  });

  it("caps options and renders honesty window", async () => {
    ...
    const { default: EmbedExamplePage } = await import(
      "../src/app/embed-example/page"
    );

    const renderer = create(<EmbedExamplePage />);
  ...
  it("renders repair trigger", async () => {
    const api = await import("../src/lib/api");
    vi.mocked(api.createSession).mockResolvedValue(baseSession);
    vi.mocked(api.runSearch).mockResolvedValue({
      ...baseSession,
